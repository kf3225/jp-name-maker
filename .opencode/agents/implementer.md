...
- **TDD（red-green）で進める**: `tdd` スキル（`.claude/skills/tdd/`）をロードし、縦スライスで「失敗テストを先に書く → 最小実装で通す」を繰り返す。実装の内部ではなく**シーム（公開境界）で合意の上**テストする。リファクタリングは実装レビュー段（impl-reviewer）で扱う（ループ外）。
